...
import { estadoDeRack, htmlDePopup } from "./popup-nodo";
import { itemsDeNodo } from "./menu-nodo";
import { itemsDeEnlace } from "./menu-enlace";
import { actualizarCarteles } from "./carteles-caida";
import { dibujarNodos } from "./render-nodos";
import { dibujarEnlaces } from "./render-enlaces";
...
export default function LeafletMapView({
  ...
  // El cuerpo vive en ./popup-nodo. Acá queda el envoltorio con la firma de
  // siempre, así renderNodes y updateDowntimeCounters siguen llamándolo igual.
  function getRackStatus(node: SavedNode) {
    return estadoDeRack(node, monitorIndexRef.current);
  }
  ...
  function createPopupContent(node: SavedNode): string {
    return htmlDePopup(node, {
      indice: monitorIndexRef.current,
      historialPing: pingHistoryRef.current,
    });
  }

  // El cuerpo vive en ./render-nodos. Sigue siendo una funcion declarada, asi
  // que renderEdges la puede referenciar aunque este definida mas abajo.
  function renderNodes(L: any, map: any) {
    return dibujarNodos(L, map, {
  ...
  // El cuerpo vive en ./render-enlaces.
  function renderEdges(L: any, map: any) {
    return dibujarEnlaces(L, map, {
  ...
  // El cuerpo vive en ./carteles-caida. Firma intacta para su unico llamador.
  function updateDowntimeCounters() {
    return actualizarCarteles({
  ...
  // El cuerpo vive en ./menu-nodo. Acá queda el envoltorio con la firma de
  // siempre: lo que antes tomaba del closure ahora viaja explícito.
  function getNodeCtxItems(nodeId: string) {
    return itemsDeNodo(nodeId, {
  ...
  // El cuerpo vive en ./menu-enlace.
  function getEdgeCtxItems(edgeId: string) {
    return itemsDeEnlace(edgeId, {
      LRef, mapRef, nodesRef, edgesRef, pushUndo, renderEdges,
      setLinkModalData, setLinkModalOpen,
    });
  }
            ...
            <DropdownItem
              ...
              onClick={() => {
                ...
                nodesRef.current = [...nodesRef.current, {
                  ...
                  custom_data: JSON.stringify({ type: "ups", upsProtocol: "snmp", upsSnmpCommunity: "public", floatPos: { fx: 0.5, fy: 0.4 } }),
                }];
                if (LRef.current) renderNodes(LRef.current, mapRef.current);
