      ...
      <div style="min-width:260px;max-width:320px;font-family:system-ui,-apple-system,sans-serif;">
        ...
        <div style="background:${statusBg};border-bottom:1px solid ${statusBorder};padding:14px 16px;display:flex;align-items:center;gap:10px;">
          ...
          <div style="flex-shrink:0;display:flex;flex-direction:column;align-items:flex-end;gap:2px;">
            <span style="font-size:10px;font-weight:800;color:${accentColor};background:${accentColor}20;padding:2px 8px;border-radius:6px;letter-spacing:0.06em;">${statusText}</span>
        ...
        <div style="padding:10px 16px 14px;">
          ...
          ${m ? `
            <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:2px 12px;">
              ...
              ${m.msg ? row("Mensaje", `<span style="max-width:140px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;vertical-align:bottom;">${m.msg}</span>`, "rgba(255,255,255,0.5)") : ""}
            </div>
          ` : `<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);border-radius:10px;padding:10px 12px;text-align:center;">
      ...
      ...(node?.icon === "ups" ? [
        {
          ...
          onClick: () => {
            ...
            if (upsCd.ip) {
              ...
              toast.info("Configura una IP en el nodo para monitorear la UPS vía SNMP", { id: "ups-noip" });
            }
          },
        },
        {
          label: "Configurar SNMP UPS",
          icon: menuIcons.Settings,
          divider: true,
          onClick: () => {
            const upsCd = safeJsonParse<NodeCustomData>(node?.custom_data);
            const current = (upsCd.upsSnmpCommunity as string) || (upsCd.snmpCommunity as string) || "public";
            const val = prompt("Comunidad SNMP para la UPS:", current);
            if (val !== null && val.trim()) {
              const idx = nodesRef.current.findIndex((n) => n.id === nodeId);
              if (idx >= 0) {
                const ncd = safeJsonParse<NodeCustomData>(nodesRef.current[idx].custom_data);
                ncd.upsSnmpCommunity = val.trim();
                nodesRef.current[idx] = { ...nodesRef.current[idx], custom_data: JSON.stringify(ncd) };
                toast.success(`SNMP community: ${val.trim()}`);
              }
            }
          },
        },
      ...
      {upsPanelState && (() => {
        ...
        return (
          <UpsPanel
            ...
            community={(upsCd.upsSnmpCommunity as string) || (upsCd.snmpCommunity as string) || "public"}
            upsName={upsNode?.label || "UPS"}
            anchorX={upsPanelState.x}
            anchorY={upsPanelState.y}
            onClose={() => setUpsPanelState(null)}
            onCommunityChange={(newComm) => {
              const idx = nodesRef.current.findIndex((n) => n.id === upsPanelState.nodeId);
              if (idx >= 0) {
                const ncd = safeJsonParse<NodeCustomData>(nodesRef.current[idx].custom_data);
                ncd.upsSnmpCommunity = newComm;
                nodesRef.current[idx] = { ...nodesRef.current[idx], custom_data: JSON.stringify(ncd) };
              }
            }}
          />
