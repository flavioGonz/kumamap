    ...
    >
      ...
      {error && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-10">
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(239,68,68,0.5)" strokeWidth="1.5">
      ...
      {/* Label overlay */}
      <div
        className="absolute inset-x-0 bottom-0 z-20 px-2.5 py-2"
          ...
          <div className="flex-1">
            ...
            {!loading && (
              <span className="text-[10px] text-[#555]">
                {filtered.length} camara{filtered.length !== 1 ? "s" : ""}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {mapNames.length > 1 && (
            <select
              ...
              className="flex-1 h-7 rounded-lg px-2 text-[10px] text-[#ededed] focus:outline-none"
              style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}
            >
              <option value="all" style={{ background: "#141414" }}>Todos</option>
              {mapNames.map((n) => (
                <option key={n} value={n} style={{ background: "#141414" }}>{n}</option>
              ))}
            </select>
          )}
          ...
          <div className="flex items-center rounded-lg overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
            {(["1", "2", "4"] as MobileGrid[]).map((g) => (
              <button
                ...
                style={{
                  background: grid === g ? "rgba(6,182,212,0.15)" : "transparent",
                  color: grid === g ? "#06b6d4" : "#555",
                  borderRight: g !== "4" ? "1px solid rgba(255,255,255,0.04)" : "none",
                }}
              >
                {g === "1" ? "1x1" : g === "2" ? "1x2" : "2x2"}
              </button>
      ...
      <main className="px-3 py-3">
        {loading && (
          <div className="flex items-center justify-center py-16">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" strokeWidth="2" className="animate-spin">
              <path d="M21 12a9 9 0 1 1-9-9c2.52 0 4.93 1 6.74 2.74L21 8" /><path d="M21 3v5h-5" />
            </svg>
          </div>
        )}
        ...
        {error && !loading && (
          <div className="flex flex-col items-center justify-center py-16">
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(239,68,68,0.5)" strokeWidth="1.5" className="mb-2">
              <path d="m22 8-6 4 6 4V8Z" /><rect width="14" height="12" x="2" y="6" rx="2" ry="2" />
            </svg>
        ...
        {!loading && !error && cameras.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16">
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="1.5" className="mb-2">
              <path d="m22 8-6 4 6 4V8Z" /><rect width="14" height="12" x="2" y="6" rx="2" ry="2" />
            </svg>
        ...
        {!loading && !error && filtered.length > 0 && (
          <div
            ...
            style={{
              gridTemplateColumns: grid === "1" ? "1fr" : grid === "2" ? "1fr" : "1fr 1fr",
            }}
          >
            {filtered.map((cam) => (
