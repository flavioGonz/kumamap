...
class KumaClient {
  ...
  addMonitor(data: Record<string, unknown>): Promise<{ ok: boolean; msg?: string; monitorID?: number }> {
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => resolve({ ok: false, msg: "Timeout adding monitor" }), 30000);
  ...
  editMonitor(data: Record<string, unknown>): Promise<{ ok: boolean; msg?: string }> {
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => resolve({ ok: false, msg: "Timeout editing monitor" }), 30000);
  ...
  deleteMonitor(monitorId: number): Promise<{ ok: boolean; msg?: string }> {
    ...
    return new Promise((resolve) => {
      const timeout = setTimeout(() => resolve({ ok: false, msg: "Timeout deleting monitor" }), 30000);
