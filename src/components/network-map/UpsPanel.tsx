...
import {
  ...
  Gauge, Power, ChevronDown, ChevronUp, Pencil, Check,
} from "lucide-react";
import { apiUrl } from "@/lib/api";
import {
  type UpsResult, type UpsHistoryPoint,
...
export default function UpsPanel({
  ip, community, upsName, onClose, onCommunityChange, anchorX, anchorY,
}: {
  ip: string; community?: string; upsName: string; onClose: () => void;
  onCommunityChange?: (newCommunity: string) => void; anchorX?: number; anchorY?: number;
}) {
  ...
  // ── Editable community ──
  const [activeCommunity, setActiveCommunity] = useState(community || "public");
  const [editingCommunity, setEditingCommunity] = useState(false);
  const [communityDraft, setCommunityDraft] = useState(activeCommunity);
  ...
  const poll = useCallback(async () => {
    ...
    try {
      const res = await fetch(apiUrl("/api/ups/poll"), {
        ...
        body: JSON.stringify({ ip, community: activeCommunity }), signal: controller.signal,
      });
      const result: UpsResult = await res.json();
      if (!mountedRef.current) return;
      setData(result);
      setError(result.reachable ? null : result.error || "No se pudo alcanzar la UPS");
      if (result.reachable && result.battery && result.output) {
        fetch(apiUrl("/api/ups/history"), {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ ip, charge: result.battery.charge, load: result.output.loadPercent ?? 0, inputV: result.input?.voltage, outputV: result.output.voltage, temp: result.battery.temperature, runtime: result.battery.runtimeMinutes, status: result.output.status }),
        }).catch(() => {});
      }
    } catch (err: any) {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await fetch(apiUrl(`/api/ups/history?ip=${encodeURIComponent(ip)}&hours=6`));
  ...
  const saveCommunity = (val: string) => {
    const v = val.trim() || "public";
    setActiveCommunity(v); setEditingCommunity(false); onCommunityChange?.(v);
    setTimeout(() => poll(), 100);
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "6px 10px", display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {loading && !data && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: 12 }}>
            ...
            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)" }}>Consultando {ip}...</span>
          </div>
        )}
        ...
        {data && !data.reachable && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: 12, textAlign: "center" }}>
            ...
            <span style={{ fontSize: 9, color: "rgba(255,255,255,0.15)" }}>{data.error || `${ip} no responde a SNMP`}</span>
      ...
      <div style={{ padding: "4px 10px", borderTop: "1px solid rgba(255,255,255,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 8, color: "rgba(255,255,255,0.12)" }}>
          <span>{ip} · SNMP</span>
          {editingCommunity ? (
            <form style={{ display: "inline-flex", alignItems: "center", gap: 2 }}
              onSubmit={(e) => { e.preventDefault(); saveCommunity(communityDraft); }}>
              <input type="text" value={communityDraft} onChange={(e) => setCommunityDraft(e.target.value)}
                onBlur={() => saveCommunity(communityDraft)}
                onKeyDown={(e) => { if (e.key === "Escape") { setCommunityDraft(activeCommunity); setEditingCommunity(false); } }}
                autoFocus
                style={{ background: "rgba(255,255,255,0.08)", border: "1px solid rgba(245,158,11,0.4)", borderRadius: 3, padding: "1px 4px", fontSize: 9, fontFamily: "monospace", color: "#f59e0b", width: 80, outline: "none" }}
                placeholder="public" />
              <button type="submit" className="cursor-pointer" style={{ color: "#22c55e", background: "none", border: "none", padding: 0, lineHeight: 1 }}><Check className="w-2.5 h-2.5" /></button>
            </form>
          ) : (
            <button onClick={() => { setCommunityDraft(activeCommunity); setEditingCommunity(true); }} className="cursor-pointer"
              style={{ display: "inline-flex", alignItems: "center", gap: 2, background: "none", border: "none", padding: "0px 3px", borderRadius: 2, color: "rgba(255,255,255,0.2)", fontFamily: "monospace", fontSize: 9, transition: "all 0.15s" }}
              ...
              title="Cambiar comunidad SNMP">
              {activeCommunity}<Pencil className="w-2 h-2" style={{ opacity: 0.5 }} />
            </button>
          )}
        </div>
